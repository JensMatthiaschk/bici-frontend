...
const mapForm = () => {
  ...
  const handleMapFormSubmit = async (e) => {
    ...
    try {
      ...
      !e.target.elements.camping.value
        ? data.append("camping", false)
        : data.append("camping", e.target.elements.camping.value);
      !e.target.elements.shower.value
        ? data.append("shower", false)
        : data.append("shower", e.target.elements.shower.value);
      !e.target.elements.repair.value
        ? data.append("repair", false)
        : data.append("repair", e.target.elements.repair.value);
      !e.target.elements.events.value
        ? data.append("events", false)
        : data.append("events", e.target.elements.events.value);
      !e.target.elements.host.value
        ? data.append("host", false)
        : data.append("host", e.target.elements.host.value);
      !e.target.elements.swim.value
        ? data.append("swim", false)
        : data.append("swim", e.target.elements.swim.value);
  ...
  return (
    <>
      <label
        htmlFor="my-modal-6"
        className="btn btn-xs btn-circle top-3 right-4 absolute"
      >
        ✕
      </label>
      <form
        ...
        <div className="form-control">
          <fieldset>
            <input
              ...
              className="input input-bordered input-md w-10/12 max-w-xs"
            />
            ...
            <textarea
              name="description"
              className="textarea textarea-bordered h-24 w-10/12 "
              placeholder="please descripe the pin here "
            ></textarea>
            <label className="label">
              <span className="label-text-alt">
                Let know other cyclist what they can find here.
                <br /> Don't leave this blank!
              </span>
            </label>
            ...
            <div className="modal-action">
              <button className="btn w-24" type="submit">
                ...
                <label
                  htmlFor="my-modal-6"
                  className="btn w-24 border-none bg-transparent"
                >
                  {" "}
                  Pin it!
                </label>
