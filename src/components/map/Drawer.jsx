...
export const Drawer = () => {
    ...
    // useEffect(() => {
    //     if (pinRatings) {
    //         pinRatings.map((e) => setRatingInput([...ratingInput, e.ratingValue]));
    //     }
    // }, [pinId]);
    ...
    useEffect(() => {
        ...
        if (pinRatings?.length) {
            ...
            let total = []
            const valueArray = pinRatings.forEach((e) =>
                total.push(+e.ratingValue)
            );
            console.log("§§§§§§§§§§", total)
            const initialValue = 0;
            if (pinRatings.length > 1) {
                const average = total.reduce((a, b) => a + b, initialValue) / pinRatings.length
                setAverageRating(average);
            }
            else { setAverageRating(pinRatings[0].ratingValue) }
        }
    }, [pinRatings]);
    ...
    console.log("TOTOTOTOTO", ratingInput)
    console.log("999999999", averageRating)
                                ...
                                return (
                                    <label>
                                        ...
                                        <FaStar
                                            className="star"
                                            color={
                                                ratingValueCurrent <= (pinRatings?.length ? averageRating : 0)
                                                    ? "rgb(251, 146, 60)"
                                                    : "grey"
                                            }
                                            size={30}
