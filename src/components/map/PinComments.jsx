...
const PinComments = (props) => {
    ...
    const { pinId, setPinId, commentData, setCommentData } = useContext(MapContext);
    ...
    return (
        <>
            ...
            <div className="flex flex-col w-full mb-4 mt-4">
                {commentData &&
                    commentData.reverse().map((e) =>
                        e.comment ? (
                            <div key={e._id} className="flex space-x-3">
                                <div className="flex flex-shrink-0">
                                    {e.userprofile.avatar_img?.aws_url ? (
                                        <img
                                            src={`${e.userprofile.avatar_img?.aws_url}`}
                                            alt="no"
                ...
                <form method="GET" onSubmit={handleSubmit} action="/comment" className="mt-0">
                    <div className="form-control">
                        ...
                        <div className="modal-action">
                            <button className="btn btn-sm w-24 relative bottom-3" type="submit">
                                comment
                            </button>
                        </div>
                    </div>
